...
from datetime import time as da_time
import httpx
import asyncio
import os
import json
...
from db_fun import store2db, deletedb
...
def time2stamp(datetime_obj) -> int:
...
with open(os.path.join(current_dir, 'settings.json'), 'r', encoding='utf8') as f:
    ...
    target_time = da_time(19, 0)
    target_datetime = datetime.combine(yestoday, target_time)
    start_time_stamp = time2stamp(target_datetime)
    target_datetime = datetime.combine(today, target_time)
    end_time_stamp = time2stamp(target_datetime)
...
def get_download_url(_user_info):
    ...
    print(f'下载图片/视频:{_user_info.count}')
    ...
    try:
        ...
        if (has_retweet) and 'cursor-top' in raw_data[0]['entryId']:      #含转推模式 所有推文已全部下载完成
            return False
        ...
        async def down_save(url, prefix, csv_info, order: int):
            if '.mp4' in url:
                ...
                if 'retweet' in prefix:
                    last_id = store2db(csv_info[2][1:], True, debug)
                    _file_name_high = f'{_user_info.save_path_high + os.sep}{csv_info[2][1:]}_{last_id}.{img_format}'
                    _file_name_low = f'{_user_info.save_path_low + os.sep}{csv_info[2][1:]}_{last_id}.{img_format}'
                else:
                    last_id = store2db(_user_info.screen_name, False, debug)
                    _file_name_high = f'{_user_info.save_path_high + os.sep}{_user_info.screen_name}_{last_id}.{img_format}'
                    _file_name_low = f'{_user_info.save_path_low + os.sep}{_user_info.screen_name}_{last_id}.{img_format}'
                ...
                try:
                    async with semaphore:
                        async with httpx.AsyncClient(proxies=None) as client:
                            ...
                            down_count += 1
                    with open(os.path.join(current_dir, os.path.join(current_dir, _file_name_high)),'wb') as f:
                        f.write(response.content)
                    img = Image.open(os.path.join(current_dir, _file_name_high))
                    ...
                    # 儲存調整後的圖片
                    img.save(os.path.join(current_dir, _file_name_low))

                    if log_output:
                        print(f'{_file_name_high}=====>下载完成')
            
                    break
                except Exception as e:
                    count += 1
                    print(f'{_file_name_high}=====>第{count}次下载失败,正在重试(多次失败时请降低main.py第11行-异步模式)')
                    print(url)
...
def main(_user_info: object):
    ...
    _user_info.save_path_high = settings['save_path'] + "high/" + str(datetime.today().date())
    _user_info.save_path_low = settings['save_path'] + "low/" + str(datetime.today().date())
...
if __name__=='__main__':
    ...
    all_user = len(following)
    if debug:
        count = 0

    _path_high = settings['save_path'] + "high/" + str(datetime.today().date())
    _path_low = settings['save_path'] + "low/" + str(datetime.today().date())
    if not os.path.exists(os.path.join(current_dir, _path_high)):   #创建文件夹
        os.makedirs(os.path.join(current_dir, _path_high))       #用户名建文件夹
        os.makedirs(os.path.join(current_dir, _path_low))       #用户名建文件夹
    else:
        deletedb(-1)
        shutil.rmtree(os.path.join(current_dir, _path_high))
        shutil.rmtree(os.path.join(current_dir, _path_low))
        os.makedirs(os.path.join(current_dir, _path_high))       #用户名建文件夹
        os.makedirs(os.path.join(current_dir, _path_low))       #用户名建文件夹


    _path_high = settings['save_path'] + "high/" + str((datetime.today() - timedelta(days=reserve_day)).date())
    _path_low = settings['save_path'] + "low/" + str((datetime.today() - timedelta(days=reserve_day)).date())
    if os.path.exists(os.path.join(current_dir, _path_high)):
        deletedb(reserve_day)
        shutil.rmtree(os.path.join(current_dir, _path_high))
        shutil.rmtree(os.path.join(current_dir, _path_low))
